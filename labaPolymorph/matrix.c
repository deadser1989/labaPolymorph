...
isSuccess printMatrixElement(const Matrix* matrix, unsigned int row, unsigned int col) {
    if (!matrix || row >= matrix->rows || col >= matrix->cols) return ERROR;
...
isSuccess addMatrix(const Matrix* matrix1, const Matrix* matrix2, Matrix* result) {
    if (!areMatricesSameSize(matrix1, matrix2) || !haveMatchingTypes(matrix1, matrix2)) return ERROR;
    createNewMatrix(matrix1->rows, matrix1->cols, matrix1->typeInfo, result);
...
isSuccess multiplyMatrix(const Matrix* matrix1, const Matrix* matrix2, Matrix* result) {
    if (!areMatricesCompatibleForMultiplication(matrix1, matrix2) || !haveMatchingTypes(matrix1, matrix2)) return ERROR;
    createNewMatrix(matrix1->rows, matrix2->cols, matrix1->typeInfo, result);
...
isSuccess transposeMatrix(Matrix* matrix) {
    Matrix temp;
    createNewMatrix(matrix->cols, matrix->rows, matrix->typeInfo, &temp);
    for (unsigned int i = 0; i < matrix->rows; ++i) {
        for (unsigned int j = 0; j < matrix->cols; ++j) {
